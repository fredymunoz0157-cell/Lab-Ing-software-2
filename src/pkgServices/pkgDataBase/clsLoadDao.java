...
public class clsLoadDao {
    ...
    public static void opLoadDatabaseToController() {
        ...
        try {
            ...
            // Se cambió 'type' por 'type_question' en la consulta y en el ResultSet
            String sqlQuestions = "SELECT id_question, question_name, question_description, optionA, optionB, optionC, optionD, rightAnswer, state, type_question, path_imagen, id_usuario "
                    + "FROM tbl_question;";
            try (PreparedStatement psQuestions = conn.prepareStatement(sqlQuestions); ResultSet rsQuestions = psQuestions.executeQuery()) {
                ...
                while (rsQuestions.next()) {
                    ...
                    String type = rsQuestions.getString("type_question"); // <-- Actualizado aquí
                    String pathImagen = rsQuestions.getString("path_imagen");
                    ...
                    // Registra la pregunta en el controlador
                    controller.opRegisterQuestion(idQuestion, qName, qDesc, optA, optB, optC, optD, rightAnswer, state, type, pathImagen, userObj);
                }
            }
